...
const sidebarSections: SidebarSection[] = [
  ...
  // MICROSERVICES SECTION
  {
    ...
    items: [
      // MICROSERVICE 1: CEMETERY & BURIAL MANAGEMENT
      {
        ...
        children: [
          ...
          { 
            ...
            icon: MdAssignment,
            children: [
              { id: "permits-dashboard", label: "Permits Dashboard", href: "/admin/permits", icon: MdDashboard },
      ...
      // MICROSERVICE 3.5: PARKS & RECREATION MANAGEMENT
      {
        id: "parks-recreation",
        label: "Parks & Recreation",
        icon: FiAward,
        children: [
          { id: "parks-dashboard", label: "Dashboard", href: "/admin/parks-recreation", icon: MdDashboard },
          { id: "amenity-reservations", label: "Amenity Reservations", href: "/admin/parks-recreation/amenity-reservations", icon: FiCalendar },
          { id: "venue-rentals", label: "Venue Rentals", href: "/admin/parks-recreation/venue-rentals", icon: FiMapPin },
          { id: "maintenance-requests", label: "Maintenance Requests", href: "/admin/parks-recreation/maintenance-requests", icon: FiTool }
        ]
      },
      // MICROSERVICE 4: FACILITY MANAGEMENT
      {
        ...
        icon: FiHome,
        children: [
          { id: "facility-dashboard", label: "Dashboard", href: "/admin/facility-management", icon: MdDashboard },
          { id: "facility-requests", label: "Facility Requests", href: "/admin/facility-management/requests", icon: FiCalendar },
          { id: "facility-list", label: "Manage Facilities", href: "/admin/facility-management/facilities", icon: FiHome },
          { id: "facility-blackouts", label: "Blackout Dates", href: "/admin/facility-management/blackouts", icon: FiClock },
          { id: "facility-analytics", label: "Analytics & Reports", href: "/admin/facility-management/analytics", icon: FiBarChart }
        ]
      }
                ...
                <div key={item.id}>
                  ...
                  {item.children ? (
                    <button
                      ...
                      className={`group relative flex items-center text-start w-full px-3 py-2.5 rounded-xl transition-all duration-200 overflow-hidden ${isParentActive(item)
                    ...
                    >
                      ...
                      {/* Left accent bar */}
                      <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-1 rounded-r-full transition-all duration-200 ${isParentActive(item)
                          ? 'h-6 bg-white'
                          : 'h-0 bg-gray-400 group-hover:h-4'
                        }`}></div>

                      {/* Icon */}
                      <div className={`relative z-10 w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 ${isParentActive(item)
                          ? ''
                          : 'group-hover:bg-white group-hover:shadow-sm'
                        }`}>
                        <item.icon size={18} className={`transition-colors duration-200 ${isParentActive(item)
                            ? 'text-white'
                      ...
                      {!isCollapsed && (
                        <>
                          <div className="relative z-10 ml-3 flex-1">
                            <span className={`font-medium text-sm transition-all duration-200 ${isParentActive(item)
                                ? 'text-white'
                          ...
                          <div className="relative z-10">
                            {expandedItems.includes(item.id) ? (
                              <FiChevronDown className={`w-4 h-4 transition-colors duration-200 ${isParentActive(item) ? 'text-white' : 'text-gray-500'
                                }`} />
                            ) : (
                              <FiChevronRight className={`w-4 h-4 transition-colors duration-200 ${isParentActive(item) ? 'text-white' : 'text-gray-500'
                                }`} />
                            )}
                          </div>
                        </>
                    ...
                    <Link
                      href={item.href || '#'}
                      prefetch={true}
                      className={`group relative flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 overflow-hidden ${isActive(item.href)
                          ? 'text-white shadow-md'
                          : 'text-gray-600 hover:text-gray-800'
                        }`}
                    ...
                    >
                      ...
                      {/* Left accent bar */}
                      <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-1 rounded-r-full transition-all duration-200 ${isActive(item.href)
                          ? 'h-6 bg-white'
                          : 'h-0 bg-gray-400 group-hover:h-4'
                        }`}></div>

                      {/* Icon */}
                      <div className={`relative z-10 w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-200 ${isActive(item.href)
                          ? ''
                          : 'group-hover:bg-white group-hover:shadow-sm'
                        }`}>
                        <item.icon size={18} className={`transition-colors duration-200 ${isActive(item.href)
                            ? 'text-white'
                            : 'group-hover:text-gray-700'
                          }`} />
                      </div>
                      ...
                      {!isCollapsed && (
                        <div className="relative z-10 ml-3 flex-1">
                          <span className={`font-medium text-sm transition-all duration-200 ${isActive(item.href)
                              ? 'text-white'
                              : 'group-hover:text-gray-800'
                            }`}>
                            {item.label}
                        ...
                        <div key={child.id}>
                          ...
                          {child.children ? (
                            <button
                              ...
                              className={`group relative flex items-center text-start w-full px-3 py-2 rounded-lg transition-all duration-200 overflow-hidden ${isParentActive(child)
                            ...
                            >
                              ...
                              {/* Left accent bar */}
                              <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-0.5 rounded-r-full transition-all duration-200 ${isParentActive(child)
                                  ? 'h-4 bg-white'
                                  : 'h-0 bg-gray-400 group-hover:h-3'
                                }`}></div>

                              {/* Icon */}
                              <div className={`relative z-10 w-6 h-6 rounded-md flex items-center justify-center transition-all duration-200 ${isParentActive(child)
                                  ? ''
                                  : 'group-hover:bg-white group-hover:shadow-sm'
                                }`}>
                                <child.icon size={14} className={`transition-colors duration-200 ${isParentActive(child)
                                    ? 'text-white'
                                    : 'group-hover:text-gray-700'
                                  }`} />
                              </div>
                              ...
                              <div className="relative z-10 ml-2 flex-1">
                                <span className={`font-medium text-xs transition-all duration-200 ${isParentActive(child)
                                    ? 'text-white'
                                    : 'group-hover:text-gray-800'
                                  }`}>
                                  {child.label}
                              ...
                              <div className="relative z-10">
                                {expandedItems.includes(child.id) ? (
                                  <FiChevronDown className={`w-3 h-3 transition-colors duration-200 ${isParentActive(child) ? 'text-white' : 'text-gray-400'
                                    }`} />
                                ) : (
                                  <FiChevronRight className={`w-3 h-3 transition-colors duration-200 ${isParentActive(child) ? 'text-white' : 'text-gray-400'
                                    }`} />
                                )}
                              </div>
                            </button>
                            ...
                            <Link
                              href={child.href || '#'}
                              prefetch={true}
                              className={`group relative flex items-center px-3 py-2 rounded-lg transition-all duration-200 overflow-hidden ${isActive(child.href)
                                  ? 'text-white shadow-sm'
                                  : 'text-gray-500 hover:text-gray-700'
                                }`}
                            ...
                            >
                              ...
                              {/* Left accent bar */}
                              <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-0.5 rounded-r-full transition-all duration-200 ${isActive(child.href)
                                  ? 'h-4 bg-white'
                                  : 'h-0 bg-gray-400 group-hover:h-3'
                                }`}></div>

                              {/* Icon */}
                              <div className={`relative z-10 w-6 h-6 rounded-md flex items-center justify-center transition-all duration-200 ${isActive(child.href)
                                  ? ''
                                  : 'group-hover:bg-white group-hover:shadow-sm'
                                }`}>
                                <child.icon size={14} className={`transition-colors duration-200 ${isActive(child.href)
                                    ? 'text-white'
                                    : 'group-hover:text-gray-700'
                                  }`} />
                              </div>
                              ...
                              <div className="relative z-10 ml-2 flex-1">
                                <span className={`font-medium text-xs transition-all duration-200 ${isActive(child.href)
                                    ? 'text-white'
                                    : 'group-hover:text-gray-800'
                                  }`}>
                                  {child.label}
                            ...
                            <div className="mt-1 ml-6 pl-3 space-y-1 border-l-2 border-orange-500/30 relative">
                              ...
                              {child.children.map((grandchild) => (
                                <Link
                                  ...
                                  prefetch={true}
                                  className={`group relative flex items-center px-3 py-1.5 rounded-md transition-all duration-200 overflow-hidden ${isActive(grandchild.href)
                                      ? 'text-white shadow-sm'
                                ...
                                >
                                  ...
                                  {/* Left accent bar */}
                                  <div className={`absolute left-0 top-1/2 transform -translate-y-1/2 w-0.5 rounded-r-full transition-all duration-200 ${isActive(grandchild.href)
                                      ? 'h-3 bg-white'
                                      : 'h-0 bg-gray-400 group-hover:h-2'
                                    }`}></div>

                                  {/* Icon */}
                                  <div className={`relative z-10 w-5 h-5 rounded-sm flex items-center justify-center transition-all duration-200 ${isActive(grandchild.href)
                                      ? ''
                                      : 'group-hover:bg-white group-hover:shadow-sm'
                                    }`}>
                                    <grandchild.icon size={12} className={`transition-colors duration-200 ${isActive(grandchild.href)
                                        ? 'text-white'
                                        : 'group-hover:text-gray-700'
                                      }`} />
                                  </div>
                                  ...
                                  <div className="relative z-10 ml-2 flex-1">
                                    <span className={`font-medium text-xs transition-all duration-200 ${isActive(grandchild.href)
                                        ? 'text-white'
                                        : 'group-hover:text-gray-700'
                                      }`}>
                                      {grandchild.label}
