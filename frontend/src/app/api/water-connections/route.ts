...
export async function GET(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/water-connections${queryParams ? `?${queryParams}` : ''}`
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.error || 'Failed to fetch water connections' },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendUrl = `${BACKEND_URL}/api/water-connections`
    ...
    if (!response.ok) {
      return NextResponse.json(
        { error: data.error || 'Failed to create water connection' },
        { status: response.status }
      )
    }
